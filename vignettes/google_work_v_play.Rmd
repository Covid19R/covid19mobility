---
title: "google_work_v_play"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{google_work_v_play}
...
Cool - we've got workplace and residential in there. I wonder if folk were spending more time at home and less time at work? The subregions data here is states/kantons/territitories, etc. Google is pretty explicit that interregional comparisons are probably not valid, but, can we look at broad trends, with each region as a data point?

First, let's just look at, well, everything!

```{r weird_ts_plot, message=FALSE, warning=FALSE}
goog_wp <- goog %>%
  filter(data_type %in% c("workplaces_percent_change_from_baseline",
...
goog_wp_wide <- pivot_wider(goog_wp,
                            ...
                            values_from = value)
ggplot(goog_wp_wide,
       aes(x = workplaces_percent_change_from_baseline, 
